...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, jittering: bool = True):
        ...
        """
        CLASS_COLORS = {
            0: [1.0, 0.0, 0.0],      # Κόκκινο για "Αυτοκίνητο" (car)
            1: [0.0, 1.0, 0.0],      # Πράσινο για "Άνθρωπος" (person)
            2: [0.0, 0.0, 1.0],      # Μπλε για "Αναβάτης" (rider)
            3: [1.0, 1.0, 0.0],      # Κίτρινο για "Μοτοσικλέτα" (motorcycle)
            4: [1.0, 0.647, 0.0],    # Πορτοκαλί για "Ποδήλατο" (bicycle)
            5: [0.502, 0.0, 0.502],  # Μωβ για "Τρένο" (train)
            6: [0.0, 1.0, 1.0],      # Κυανό για "Λεωφορείο" (bus)
            7: [0.502, 0.502, 0.502] # Γκρι για "Φορτηγό" (truck)
        }
        ...
        # Σταθερά χρώματα ανά κλάση
        if self._instance_mode == ColorMode.SEGMENTATION:
            colors = [CLASS_COLORS[c] for c in classes]
        else:
            colors = [CLASS_COLORS[c] for c in classes]

        alpha = 0.8 if self._instance_mode == ColorMode.SEGMENTATION else 0.5
